...
describe('ClubService', () => {
  ...
  it('findOne should throw an exception for an invalid club', async () => {
    await expect(() => service.findOne("0")).rejects.toHaveProperty("message", "El club con el ID indicado no fue encontrado")
  });
  ...
  it('create should throw an exception for an invalid description', async () => {
    ...
    await expect(() => service.create(club)).rejects.toHaveProperty("message", "La descripción no puede tener más que 100 caracteres")
  });

  it('update should modify a club', async () => {
    const club: ClubEntity = clubsList[0];
    club.nombre = "New name";
    club.fecha_fundacion = faker.date.past();
    club.descripcion = "New description";
    club.imagen = "New-image-URL";
  ...
  it('update should throw an exception for an invalid club', async () => {
    ...
    club = {
      ...club, nombre: "New name", fecha_fundacion: faker.date.past(), imagen: "New-image-url", descripcion: "New description"
    }
    await expect(() => service.update("0", club)).rejects.toHaveProperty("message", "El club con el ID indicado no fue encontrado")
  });
  ...
  it('delete should throw an exception for an invalid club', async () => {
    const club: ClubEntity = clubsList[0];
    await expect(() => service.delete("0")).rejects.toHaveProperty("message", "El club con el ID indicado no fue encontrado")
  });
});
